...
var FEATURE_NAME = args.feature || args.name || 'unnamed-feature'
var DESCRIPTION = args.description || 'Implement ' + FEATURE_NAME
...
var plan = await agent(
  ...
  { label: 'Plan ' + FEATURE_NAME, phase: 'Plan', model: 'opus' }
)
...
var hasBackend = plan.indexOf('entity') >= 0 || plan.indexOf('controller') >= 0 || plan.indexOf('service') >= 0
...
var hasFrontend = plan.indexOf('component') >= 0 || plan.indexOf('page') >= 0 || plan.indexOf('hook') >= 0
...
var needsMigration = plan.indexOf('migration') >= 0 || plan.indexOf('entity') >= 0 || plan.indexOf('field') >= 0
...
if (needsMigration) {
  await agent(
    ...
    '2. Generate migration via npm run migration:generate\n' +
    '3. Run migration via npm run migration:run\n' +
    '4. Verify no errors',
